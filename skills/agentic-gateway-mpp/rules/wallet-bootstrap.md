...
**This is the mandatory entry point for MPP gateway requests when no API key is available.** No data can be fetched until wallet setup is complete. If the user asks a blockchain question and no wallet is configured (and no API key is set), redirect them here first. Do NOT mention the API key or suggest obtaining one — go straight to wallet setup.
...
Use the private key for auth token generation (see [authentication](authentication.md)) and payment creation (see [making-requests](making-requests.md)).
