...
**Base URL**: `https://mpp.alchemy.com/prices/v1/`
...
curl -s -H "Authorization: SIWE $TOKEN" \
  "https://mpp.alchemy.com/prices/v1/tokens/by-symbol?symbols=ETH&symbols=BTC"
```
...
```bash
curl -s -X POST "https://mpp.alchemy.com/prices/v1/tokens/by-address" \
...
curl -s -X POST "https://mpp.alchemy.com/prices/v1/tokens/historical" \
  -H "Content-Type: application/json" \
  -H "Authorization: SIWE $TOKEN" \
  -d '{
    "symbol": "ETH",
